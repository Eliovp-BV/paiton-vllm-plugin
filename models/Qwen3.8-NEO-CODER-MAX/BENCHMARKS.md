...
The faster experimental rc3 GEMM profile was rejected after it exceeded the
preset full-logit bounds, despite unchanged greedy token IDs. The corrected
candidate restored exact agreement. No rejected result is used in these tables.
...
The [archived initial native Paiton/vLLM results](BENCHMARKS-v1.0.0.md) and
[release metadata](paiton-release-v1.0.0.json) remain available for provenance
and rollback. That initial implementation was not stock vLLM. Its different
activation arithmetic is documented in the numerical comparison above.
