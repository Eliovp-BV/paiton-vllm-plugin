...
with av.open(str(a.clip)) as container:
    for index,frame in enumerate(container.decode(video=0)):
        ...
        seen[digest]=index
        thumbnail=ImageOps.contain(Image.fromarray(arr),(216,120))
        canvas=Image.new('RGB',(216,120),(23,25,29));canvas.paste(thumbnail,((216-thumbnail.width)//2,(120-thumbnail.height)//2))
        small=np.asarray(canvas)
        ...
        if index in (0,24,48,72,96,120):Image.fromarray(arr).save(a.output/f'frame-{index:03}.png')
rows=(len(thumbs)+4)//5
