...
The source-build Docker recipe uses PyAV 18.1.0 (BSD-3-Clause), FFmpeg 8.1.2 (configured LGPLv3, with GPL and nonfree components disabled), LAME 3.100 (LGPL), and Opus 1.6.1 (BSD-style license and upstream patent notices). NASM 2.16.03 and Cython 3.3.0 are pinned build tools with their own upstream licenses. The image carries the corresponding media source archives, exact build recipe and copied notices under `/opt/meeting-media/share/paiton-media`; preserve these when distributing the image. The source archive contains additional optional FFmpeg components that are not enabled in the binaries. This build does not apply the PyAV vendor patch that reclassifies x264/x265 in FFmpeg's configure checks, and does not enable either encoder. The earlier benchmark image used the upstream PyAV binary wheel and is not the proposed source-build release image.
