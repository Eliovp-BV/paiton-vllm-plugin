...
# Adapted stock vLLM0.28 control flow, commit2cf0a6915ce544dc493a0990f2ea38d81601128a.
# Changes: native state preparation and native speculative recurrence dispatch.
# Stock convolution, prefill, ordinary-decode and output assembly are preserved.
import torch
from vllm.forward_context import get_forward_context
from vllm.model_executor.layers.mamba.gdn.qwen_gdn_linear_attn import (
    GDNAttentionMetadata, is_conv_state_dim_first, causal_conv1d_update,
    causal_conv1d_fn, fused_post_conv_prep, fused_sigmoid_gating_delta_rule_update,
)
...
def forward_core(
    ...
    self._prepare_native_replay(attn_metadata)
    ...
    native_prefill_inputs = None
    conv_runtime = getattr(self, 'paiton_conv_prefill', None)
    if conv_runtime is not None and attn_metadata.num_prefills and not attn_metadata.num_decodes and spec_sequence_masks is None:
        native_prefill_inputs = conv_runtime.prepare(self, mixed_qkv, a, b, conv_state, conv_weights, attn_metadata)
    ...
    if native_prefill_inputs is not None:
        pass  # Native preparation already updated convolution state and produced q/k/v/g/beta.
    ...
    if attn_metadata.num_prefills > 0:
        ...
        if native_prefill_inputs is not None:
            query_non_spec, key_non_spec, value_non_spec, g_non_spec, beta_non_spec = native_prefill_inputs
        else:
            (
                query_non_spec,
                key_non_spec,
    ...
    if spec_sequence_masks is not None:
        core_attn_out_spec, last_recurrent_state = (
            self._native_speculative_update(
                A_log=self.A_log,
                a=a_spec,
                b=b_spec,
                dt_bias=self.dt_bias,
