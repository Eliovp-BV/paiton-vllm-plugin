...
import threading
import time
from paiton_gdn_merged_prefill_policy import Policy, FLAG

TARGET='radiance_mxfp4_fp8'
ATTENTION_TARGET='radiance_r4d_attn'
ROOT=Path('/opt/paiton/runtime/gdn-merged-prefill')
LIB=AUDIT=POLICY=None
CALLS=FALLBACKS=0
SHADOW=os.environ.get('PAITON_MXFP4_GDN_MERGED_PREFILL_SHADOW','0')=='1'
...
def load():
    ...
    if hashlib.sha256(Path(__import__('paiton_gdn_merged_prefill_policy').__file__).read_bytes()).hexdigest()!=manifest['policy_sha256']:
    ...
    print('[paiton.gdn_merged_prefill] '+json.dumps({'policy':POLICY.snapshot(),'shadow':SHADOW}),file=sys.stderr,flush=True)

def wrap(module):
    if getattr(module,'_paiton_gdn_merged_prefill_wrapped',False):return
    original=module.launch_at
    def launch(a,w,ws,wr,scale,y,m,n,k,stream):
        global CALLS,FALLBACKS
        if (m,n,k)!=(4089,16480,5120):
            FALLBACKS+=1
        ...
        if CALLS in (1,64,256,512,4096):
            print('[paiton.gdn_merged_prefill] native launches='+str(CALLS),file=sys.stderr,flush=True)
        return None
    module.launch_at=launch
    module._paiton_gdn_merged_prefill_wrapped=True
...
def verify_source(module):
    ...
    if actual!=expected[module.__name__]:
        raise RuntimeError('Merged GDN external source changed: '+module.__name__)

def initialize_at_metadata_construction(module):
    """The pinned builder allocates GPU scratch before model graph capture."""
    original=module.R4DAttentionMetadataBuilder.__init__
    import functools
    ...
    def initialize(self,*args,**kwargs):
        ...
        if LIB is None:load()
        return result
    module.R4DAttentionMetadataBuilder.__init__=initialize
...
class Loader:
    ...
    def exec_module(self,module):
        ...
        verify_source(module)
        if module.__name__==TARGET:wrap(module)
        else:initialize_at_metadata_construction(module)

class Finder(importlib.abc.MetaPathFinder):
    def find_spec(self,fullname,path=None,target=None):
        if fullname not in (TARGET,ATTENTION_TARGET):return None
        after=False
...
if requested in ('1','on','true'):
    if ATTENTION_TARGET in sys.modules:
        raise RuntimeError('Install merged GDN hook before attention backend import')
    if TARGET in sys.modules:
        verify_source(sys.modules[TARGET]);wrap(sys.modules[TARGET])
