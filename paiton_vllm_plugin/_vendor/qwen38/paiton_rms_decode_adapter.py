...
import threading
import time
from paiton_rms_decode_policy import Policy, FLAG

TARGET='radiance_mxfp4_fp8'
ROOT=Path('/opt/paiton/runtime/rms-decode')
LIB=AUDIT=POLICY=None
CALLS=FALLBACKS=0
SHADOW=os.environ.get('PAITON_TARGET_RMS_DECODE_SHADOW','0')=='1'
...
def load():
    ...
    if hashlib.sha256(Path(__import__('paiton_rms_decode_policy').__file__).read_bytes()).hexdigest()!=manifest['policy_sha256']:
    ...
    LIB.paiton_rms_decode_arch.restype=C.c_char_p
    if LIB.paiton_rms_decode_abi_version()!=1 or LIB.paiton_rms_decode_arch()!=b'gfx1201':raise ValueError('Native RMS ABI mismatch')
    LIB.paiton_rms_decode.argtypes=[C.c_void_p]*6+[C.c_int]*2+[C.c_float,C.c_void_p]
    LIB.paiton_rms_decode.restype=C.c_int
    if SHADOW:
        ...
        AUDIT.paiton_rms_decode_audit_compare.argtypes=[C.c_void_p]*9+[C.c_int]*2+[C.c_void_p]
        AUDIT.paiton_rms_decode_audit_report.argtypes=[C.POINTER(C.c_ulonglong)]
        check(AUDIT.paiton_rms_decode_audit_init())
        ...
        def report():
            ...
            for stage in ('warmup','final'):
                ...
                counts=(C.c_ulonglong*14)();rc=AUDIT.paiton_rms_decode_audit_report(counts)
                value={'hip_status':rc,'completed_comparisons':counts[0],'fp8_different_bytes':counts[1],
                       ...
                       'residual_different_values':counts[4],'nonfinite_residuals':counts[5],'m8_comparisons':counts[6],'m64_comparisons':counts[7],'nonfinite_y':counts[8],'nonfinite_input_residual':counts[9],'nonfinite_weights':counts[10],'residual_nonfinite_from_finite_inputs':counts[11],'reference_nonfinite_residuals':counts[12],'candidate_nonfinite_residuals':counts[13],'python_launches':CALLS,'fallbacks':FALLBACKS,'policy':POLICY.snapshot()}
                path=directory/(stage+'-audit.json');tmp=path.with_suffix('.tmp');tmp.write_text(json.dumps(value,indent=2)+'\n');tmp.replace(path)
        threading.Thread(target=report,daemon=True).start()
    print('[paiton.rms_decode] '+json.dumps({'policy':POLICY.snapshot(),'shadow':SHADOW}),file=sys.stderr,flush=True)

def wrap(module):
    if getattr(module,'_paiton_rms_decode_wrapped',False):return
    ...
    def launch(y,residual,weight,q,scale,residual_out,M,K,eps,stream,tiled=0):
        ...
        if M not in (8,64) or (K,tiled)!=(5120,0) or eps not in (1.e-6,1.e-5):
            FALLBACKS+=1
        ...
        if SHADOW:
            # The reference must not overwrite any input before the candidate
            # sees it. Shadow temporaries alone do not establish same-input data.
            buffers=((y,M*K*2),(residual,M*K*2),(weight,K*2),
                     (q,M*K),(scale,M*4),(residual_out,M*K*2))
            for i in range(3,6):
            ...
            temporary=torch.empty((M,K),device='cuda',dtype=torch.uint8)
            ...
            original(*args)
        check(LIB.paiton_rms_decode(y,residual,weight,output,scale_output,res_output,M,K,eps,stream))
        CALLS+=1
        if SHADOW:check(AUDIT.paiton_rms_decode_audit_compare(q,output,scale,scale_output,residual_out,res_output,y,residual,weight,M,K,stream))
        if CALLS in (1,64,256,512,4096):print('[paiton.rms_decode] native launches='+str(CALLS),file=sys.stderr,flush=True)
        return None
    module.launch_add_rms_quant=launch
    module._paiton_rms_decode_wrapped=True
...
if requested not in ('0','1','off','on','false','true'):raise ValueError('Invalid RMS decode enable flag')
