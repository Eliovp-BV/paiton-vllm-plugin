...
FLAG = 'PAITON_TARGET_RMS_DECODE'
VERSION = 'target-rms-decode-v1'
...
class Policy:
    ...
    def from_environment(cls, env, manifest):
        ...
        if contract != (VERSION, 'gfx1201', [[8, 5120], [64, 5120]], 256, 1, 'row_major_fp8'):
    ...
    def allows(self, m, n, *, arch, tiled, eps):
        return (self.enabled and m in (8, 64) and n == 5120 and arch == 'gfx1201'
                and tiled == 0 and eps in (1.e-6, 1.e-5))

    def snapshot(self):
        return {'version': self.version, 'enabled': self.enabled,
                'artifact_sha256': self.artifact_sha256, 'flag': FLAG,
                'shape_mn': [[8, 5120], [64, 5120]], 'layout': 'row_major_fp8',
                'reduction_threads': 256, 'epsilons': [1.e-6, 1.e-5],
