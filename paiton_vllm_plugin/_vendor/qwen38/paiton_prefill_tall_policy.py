...
FLAG = 'PAITON_MXFP4_PREFILL_TALL_TILE'
VERSION = 'mxfp4-prefill-tall-v1'
...
class Policy:
    ...
    def from_environment(cls, env, manifest):
        ...
        if (value in ('1', 'on', 'true') and
                any(env.get(flag, '0') in ('1', 'on', 'true') for flag in
                    ('PAITON_MXFP4_PREFILL_GROUP4', 'PAITON_MXFP4_PREFILL_PIPELINE'))):
            raise ValueError('Select only one target-prefill implementation')
        if (manifest.get('version'), manifest.get('arch'), manifest.get('shape_mnk'),
            manifest.get('group_m'), manifest.get('abi')) != (VERSION, 'gfx1201', [4089, 34816, 5120], 4, 1):
    ...
    def allows(self, m, n, k, *, arch, layout, partitions=1):
        return (self.enabled and (m, n, k) == (4089, 34816, 5120)
    ...
    def snapshot(self):
        return {'version': self.version, 'enabled': self.enabled,
                ...
                'shape_mnk': [4089, 34816, 5120], 'partitions': 1, 'tile_mnk': [512,64,64],
