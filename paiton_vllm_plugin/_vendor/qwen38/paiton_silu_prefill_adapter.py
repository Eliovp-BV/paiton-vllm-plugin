...
import threading
import time
from paiton_silu_prefill_policy import Policy, FLAG

TARGET='radiance_mxfp4_fp8'
ROOT=Path('/opt/paiton/runtime/silu-prefill')
LIB=AUDIT=POLICY=None
CALLS=FALLBACKS=0
SHADOW=os.environ.get('PAITON_TARGET_SILU_PREFILL_SHADOW','0')=='1'
...
def load():
    ...
    if hashlib.sha256(Path(__import__('paiton_silu_prefill_policy').__file__).read_bytes()).hexdigest()!=manifest['policy_sha256']:
    ...
    LIB.paiton_silu_prefill_arch.restype=C.c_char_p
    if LIB.paiton_silu_prefill_abi_version()!=1 or LIB.paiton_silu_prefill_arch()!=b'gfx1201':raise ValueError('Native SiLU ABI mismatch')
    LIB.paiton_silu_prefill.argtypes=[C.c_void_p]*3+[C.c_int]*3+[C.c_void_p]
    LIB.paiton_silu_prefill.restype=C.c_int
    if SHADOW:
        ...
        AUDIT.paiton_silu_audit_compare.argtypes=[C.c_void_p]*4+[C.c_int]*2+[C.c_void_p]
        AUDIT.paiton_silu_audit_report.argtypes=[C.POINTER(C.c_ulonglong)]
        check(AUDIT.paiton_silu_audit_init())
        ...
        def report():
            ...
            for stage in ('warmup','final'):
                ...
                counts=(C.c_ulonglong*4)();rc=AUDIT.paiton_silu_audit_report(counts)
    ...
    print('[paiton.silu] '+json.dumps({'policy':POLICY.snapshot(),'shadow':SHADOW}),file=sys.stderr,flush=True)

def wrap(module):
    if getattr(module,'_paiton_silu_wrapped',False):return
    original=module.launch_silu_mul_quant
    def launch(gu,q,scale,M,N,stream,tiled=0):
        global CALLS,FALLBACKS
        if (M,N,tiled)!=(4089,17408,1):
            FALLBACKS+=1
        ...
        if SHADOW:
            ...
            temporary=torch.empty(((M+15)//16*16,N),device='cuda',dtype=torch.uint8)
        ...
        check(LIB.paiton_silu_prefill(gu,output,scale_output,M,N,0,stream))
        CALLS+=1
        if SHADOW:check(AUDIT.paiton_silu_audit_compare(q,output,scale,scale_output,M,N,stream))
        if CALLS in (1,64,256,512,4096):print('[paiton.silu] native launches='+str(CALLS),file=sys.stderr,flush=True)
        return None
    module.launch_silu_mul_quant=launch
    module._paiton_silu_wrapped=True
