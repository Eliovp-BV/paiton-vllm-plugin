...
def select_2d_config(
    ...
    # --- RADIANCE LDS fit (2D): shrink the staged K/V tile into the R9700's 64 KiB LDS ---
    _rad_el = 2 if kv_cache_dtype in (torch.bfloat16, torch.float16) else 1
    _rad_hs = triton.next_power_of_2(head_size)
    while num_stages_2d > 1 and TILE_SIZE * _rad_hs * _rad_el * num_stages_2d + 256 > 65536:
        num_stages_2d -= 1
    while TILE_SIZE > 16 and TILE_SIZE * _rad_hs * _rad_el * num_stages_2d + 256 > 65536:
        TILE_SIZE //= 2
...
def select_3d_config(
    ...
    # --- RADIANCE LDS fit (3D): shrink the staged K/V tile into the R9700's 64 KiB LDS ---
    _rad_el = 2 if kv_cache_dtype in (torch.bfloat16, torch.float16) else 1
    _rad_hs = triton.next_power_of_2(head_size)
    while attn_stages > 1 and TILE_SIZE * _rad_hs * _rad_el * attn_stages + 256 > 65536:
        attn_stages -= 1
    while TILE_SIZE > 16 and TILE_SIZE * _rad_hs * _rad_el * attn_stages + 256 > 65536:
        TILE_SIZE //= 2
